...
tensor = gc.load_raster(catalog, aoi, band_indexes=[2, 3, 4, 8])
```
...
```python
catalog = gc.build_xarray_catalog(
    filepaths=glob("/data/modis/MOD13A2_*.nc"),
    target_crs="EPSG:4326",
    data_vars=["NDVI"],
    time_var="time",
)
ds = gc.load_xarray(catalog, aoi, data_vars=["NDVI"])
```
...
```python
catalog = gc.build_vector_catalog(
    filepaths=glob("/labels/*.gpkg"),
    target_crs="EPSG:32629",
)
labels = gc.load_vector(
    catalog, aoi,
...
paired  = gc.intersect(imagery, labels)  # rows that overlap in space AND time
...
iterable of `GeoSlice`s) that a tiling patcher can consume. The
canonical consumer is `geotoolz.patch.SpatialPatcher`, but any code
that walks `domain.slices()` works.
...
`DuckDBGeoCatalog` (v0.2; `pip install 'geocatalog[duckdb]'`) wraps a
